  ...
  "A ProjectId" should {
    ...
    "be constructed properly" in {
      val list = Vector.fill(100) {
        applyRef[Id](s"https://nexus.example.com/projects/${genString(5)}/${genString(5)}").toOption.get
      }
      forAll(list) { el =>
    ...
    "fail to contstruct when namespace doesn't match" in {
      val invalidId =
        applyRef[Id](s"https://nexus.example.com/notprojects//${genString(5)}/${genString(5)}").toOption.get
      applyRef[ProjectId](invalidId).left.value shouldEqual s"ValidConvertibleProjectUri predicate failed: ${invalidId.value} doesn't start with required namespace: ${projectNamespace.value}"
    }
