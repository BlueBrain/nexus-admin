...
object LinksQueryResults {
  ...
  final implicit def encodeLinksQueryResults[A](
      implicit E: Encoder[QueryResult[A]],
      L: Encoder[Links]
  ): Encoder[LinksQueryResults[A]] = {
    Encoder.encodeJson.contramap { response =>
      val json = Json.obj(
        nxv.total.reference.value   -> Json.fromLong(response.response.total),
        nxv.results.reference.value -> response.response.results.asJson,
        nxv.links.reference.value   -> response.links.asJson
      )
