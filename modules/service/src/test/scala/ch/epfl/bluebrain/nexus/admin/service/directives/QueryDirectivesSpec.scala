...
class QueryDirectivesSpec
  ...
  "A searchQueryParams directive" should {
    ...
    "extract deprecated and q query params when provided" in {
      Get("/?_deprecated=false&_q=something") ~> route {
    ...
    "reject when an invalid JSON-LD output format is provided" in {
      Get("/?_format=foobar") ~> route() ~> check {
    ...
    "reject when invalid JSON" in {
      Get(s"/?_context=${URLEncoder.encode("""{"a": b}]""", "UTF-8")}") ~> route() ~> check {
        status shouldEqual StatusCodes.BadRequest
    ...
    "extract fields, pagination, q and deprecated when provided" in {
      Get("/?_deprecated=true&_q=something&_from=1&_size=30&_fields=nxv:one,schema:two,rdf:three,,") ~> route {
        Response(
          Pagination(1L, 30),
          QueryPayload(
    ...
    "extract sort when provided" in {
      ...
      Get(s"/?_sort=-nxv:_createdAtTime,${rdfType},,,") ~> route {
        val expectedSort =
    ...
    "merge context when specified as JSON object" in {
      Get(s"/?_context=${URLEncoder.encode(JsonLD(addedContext).contextValue.noSpaces, "UTF-8")}") ~> route {
