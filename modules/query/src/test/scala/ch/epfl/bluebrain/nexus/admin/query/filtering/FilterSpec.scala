...
import ch.epfl.bluebrain.nexus.admin.query.filtering.Expr.{ComparisonExpr, InExpr, LogicalExpr}
import ch.epfl.bluebrain.nexus.admin.query.filtering.Filter._
import ch.epfl.bluebrain.nexus.admin.query.filtering.FilterSpec._
import ch.epfl.bluebrain.nexus.admin.query.filtering.Op._
import ch.epfl.bluebrain.nexus.admin.query.filtering.PropPath.{AlternativeSeqPath, PathZeroOrOne, SeqPath, UriPath}
...
class FilterSpec extends WordSpecLike with Matchers with Resources with EitherValues {
  ...
  val nexusBaseVoc: Uri = nxv.namespaceBuilder.value
...
object FilterSpec {
  def contextAndFilter(filterWithContext: Json): (Json, Json) = {
    val filter = filterWithContext.hcursor.get[Json]("filter").toOption.getOrElse(Json.obj())
    filter -> Json.obj("@context" -> JsonLD(filterWithContext).contextValue)
  }
