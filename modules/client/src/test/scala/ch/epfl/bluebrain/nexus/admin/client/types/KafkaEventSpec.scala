...
class KafkaEventSpec extends WordSpecLike with Matchers with Inspectors with Resources with EitherValues {
  ...
  "Kafka events" should {
    ...
    val base     = Iri.absolute("https://nexus.example.ch").right.value
    val mappings = Map(
      "nxv"    -> Iri.absolute("https://bluebrain.github.io/nexus/vocabulary/").right.value,
      "shacl"  -> Iri.absolute("https://bluebrain.github.io/nexus/schemas/shacl").right.value,
      "schema" -> Iri.absolute("http://schema.org/").right.value
    )
    ...
    val list = List(
      jsonContentOf("/kafka/proj-created.json") -> ProjectCreated(projId,
                                                                  ...
                                                                  meta,
                                                                  projCreated),
      jsonContentOf("/kafka/proj-created-no-pm.json") -> ProjectCreated(projId,
                                                                        "some-id",
                                                                        projUuid,
                                                                        orgUuid,
                                                                        42L,
                                                                        meta,
                                                                        projCreated.copy(prefixMappings = Map.empty)),
