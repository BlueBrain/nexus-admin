...
class KafkaEventSpec extends WordSpecLike with Matchers with Inspectors with Resources with EitherValues {
  ...
  "Kafka events" should {
    ...
    val list = List(
      jsonContentOf("/kafka/proj-created.json")    -> ProjectCreated(projId, projUuid, orgUuid, 42L, meta, createdJson),
      jsonContentOf("/kafka/proj-updated.json")    -> ProjectUpdated(projId, projUuid, orgUuid, 43L, meta, updatedJson),
